...
from cc_code.cli_commands import find_matching_slash_commands, try_handle_local_command
...
from cc_code.manage_cli import maybe_handle_management_command
from cc_code.model_registry import create_model_adapter, detect_provider, format_model_status, format_model_list
...
def _render_banner(runtime: dict | None, cwd: str, permission_summary: list[str], counts: dict[str, int]) -> str:
    ...
    lines = [
        "╔══════════════════════════════════════════════════════════╗",
        "║  🤖 CC-Coder - Your Terminal Coding Assistant           ║",
        "╠══════════════════════════════════════════════════════════╣",
        f"║  Model: {model:<46} ║",
        f"║  CWD: {cwd:<50} ║",
    ]
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="CC-Coder - A lightweight terminal coding assistant",
        add_help=True,
    )
    parser.add_argument(
        "--resume",
    ...
    # Initialize logging
    from cc_code.logging_config import setup_logging
    ...
    if args.install:
        from cc_code.install import main as install_main
        install_main()
        return
    ...
    from cc_code.user_profile import UserProfileManager
    ...
    # 显示快速入门指南
    if not sys.stdin.isatty() or os.environ.get("CC_CODE_SHOW_GUIDE", "1") == "1":
