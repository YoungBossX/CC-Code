...
class ContextManager:
    ...
    def compact_messages(self) -> list[dict[str, Any]]:
        ...
        """
        # Use the dynamic multi-level trigger for compaction instead of
        # the fixed stats.should_compact flag. Tests call `should_auto_compact()`
        # and expect `compact_messages()` to honor that decision.
        if not self.should_auto_compact():
            return self.messages
        
        # Recompute stats now that we committed to compaction
        stats = self.get_stats()
    ...
    def _finalize_compaction(
        ...
        # Identify removed messages by value (dict equality) rather than
        # object id, because some phases create new dicts/trimmed copies.
        ...
        # Record compaction (do not mutate `self.messages` here — callers
        # may depend on comparing the returned compacted list to the
        # pre-compaction `self.messages` length; tests expect this behavior)
        ...
        # NOTE: intentionally do NOT overwrite `self.messages` or rebuild
        # the token cache here so callers can compare pre/post lists. The
        # compaction result is returned; if callers want to persist it they
        # may assign it back to `manager.messages` explicitly.
        return compacted
...
def save_context_state(manager: ContextManager) -> None:
    ...
    state_path = CC_CODE_DIR / "context_state.json"
