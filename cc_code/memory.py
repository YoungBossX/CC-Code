...
- User memory (~/.cc-code/memory/) - cross-project, persistent
- Project memory (.cc-code-memory/) - shared across sessions, can be versioned
- Local memory (.cc-code-memory-local/) - project-specific, not checked in
...
def _auto_classify_content(content: str) -> tuple[str, list[str]]:
    ...
    # Heuristic: if the content includes clear code markers, prefer code-pattern
    # even if other categories have similar scores. This helps classify short
    # snippets like "Use def foo(): ..." as code patterns.
