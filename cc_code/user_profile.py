...
Supports two scopes:
- Global: ~/.cc-code/USER.md  (applies across all projects)
- Project: .cc-code/USER.md   (project-specific overrides)
...
class UserProfileManager:
    ...
    def __init__(self, cwd: str | Path | None = None):
        ...
        self._global_path = CC_CODE_DIR / "USER.md"
        self._project_path = Path(cwd or Path.cwd()) / ".cc-code" / "USER.md"
...
def handle_user_command(args: str, cwd: str | Path | None = None) -> str:
    ...
    if not subcmd or subcmd == "show":
        ...
        if not prompt_section:
            return "No user profile configured. Create ~/.cc-code/USER.md or .cc-code/USER.md"
