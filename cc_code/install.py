"""Interactive installer for CC-Coder.
...
def _install_launcher_script() -> str | None:
    ...
    if sys.platform == "win32":
        # Windows: Use ~/.cc-code/bin with .bat script
        target_bin_dir = CC_CODE_DIR / "bin"
        launcher_path = target_bin_dir / "cc-coder.bat"
        python_exe = sys.executable.replace("/", "\\")
        launcher_script = "\r\n".join([
            "@echo off",
            "REM CC-Coder Launcher for Windows",
            f'"{python_exe}" -m cc_code.main %*',
            "",
        ])
        launcher_command = "cc-coder.bat"
    elif sys.platform == "darwin":
        # macOS: Use ~/.local/bin with bash script (also works with zsh)
        target_bin_dir = home / ".local" / "bin"
        launcher_path = target_bin_dir / "cc-coder"
        python_exe = sys.executable
        launcher_script = "\n".join([
            "#!/usr/bin/env bash",
            "# CC-Coder Launcher for macOS",
            "# Works with bash, zsh, and other shells",
            "set -euo pipefail",
            f'exec "{python_exe}" -m cc_code.main "$@"',
            "",
        ])
        launcher_command = "cc-coder"
    else:
    ...
    try:
        ...
        print(f"\n⚠️  无法安装启动器脚本: {e}")
        print("你可以手动创建启动器脚本来调用 CC-Coder。")
        return None
...
def main() -> None:
    ...
    print("=" * 60)
    print()
    print(f"配置会写入: {CC_CODE_SETTINGS_PATH}")
    ...
    try:
        ...
        print(f"✅ 配置已保存到: {CC_CODE_SETTINGS_PATH}")
    except OSError as e:
        print(f"\n❌ 保存配置失败: {e}")
        sys.exit(1)
    ...
    print("=" * 60)
    print()
    print("📁 配置文件:", CC_CODE_SETTINGS_PATH)
    if launcher_result:
    ...
    print()
    print("  Windows:")
    print("    cc_code.bat               (如果已添加 PATH)")
    print("    python -m cc_code.main    (通用方式)")
    print()
    print("  macOS:")
    print("    cc_code-py                (如果已添加 PATH)")
    print("    python3 -m cc_code.main   (通用方式)")
    print()
    print("  Linux:")
    print("    cc_code-py                (如果已添加 PATH)")
    print("    python3 -m cc_code.main   (通用方式)")
    print()
    print("感谢使用 CC-Coder Python！🎉")
    print()
