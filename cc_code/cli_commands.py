...
from cc_code.config import (
    CLAUDE_SETTINGS_PATH,
    CC_CODE_MCP_PATH,
    CC_CODE_PERMISSIONS_PATH,
    CC_CODE_SETTINGS_PATH,
    load_runtime_config,
...
SLASH_COMMANDS = [
    ...
    SlashCommand("/history", "/history", "Show recent prompt history from ~/.cc-code/history.json."),
    ...
    SlashCommand("/model", "/model <model-name>", "Persist a model override into ~/.cc-code/settings.json."),
    SlashCommand("/config-paths", "/config-paths", "Show cc-code and Claude fallback settings paths."),
    ...
    SlashCommand("/permissions", "/permissions", "Show cc-code permission storage path."),
    SlashCommand("/exit", "/exit", "Exit cc-code."),
...
def try_handle_local_command(user_input: str, tools=None, cwd: str | None = None) -> str | None:
    ...
    if user_input == "/config-paths":
        return "\n".join(
            [
                f"cc-code settings: {CC_CODE_SETTINGS_PATH}",
                f"cc-code permissions: {CC_CODE_PERMISSIONS_PATH}",
                f"cc-code mcp: {CC_CODE_MCP_PATH}",
                f"compat fallback: {CLAUDE_SETTINGS_PATH}",
            ]
        )

    if user_input == "/permissions":
        return f"permission store: {CC_CODE_PERMISSIONS_PATH}"

    if user_input == "/skills":
        skills = tools.get_skills() if tools else []
        if not skills:
            return "No skills discovered. Add skills under ~/.cc-code/skills/<name>/SKILL.md, .cc-code/skills/<name>/SKILL.md, .claude/skills/<name>/SKILL.md, or ~/.claude/skills/<name>/SKILL.md."
        return "\n".join(
    ...
    if user_input == "/state":
        try:
            from cc_code.state import handle_state_command
    ...
    if user_input == "/context":
        ...
        try:
            from cc_code.context_manager import load_context_state
    ...
    if user_input == "/mcp":
        ...
        if not servers:
            return "No MCP servers configured. Add mcpServers to ~/.cc-code/settings.json, ~/.cc-code/mcp.json, or project .mcp.json."
        lines = []
    ...
    if user_input == "/status":
        ...
        from cc_code.model_registry import detect_provider
    ...
    if user_input == "/model":
        try:
            runtime = load_runtime_config()
            from cc_code.model_registry import format_model_status
    ...
    if user_input.startswith("/model "):
        ...
        if not arg:
            from cc_code.model_registry import format_model_list
        ...
        from cc_code.model_registry import Provider, format_model_list
        for p in Provider:
            if arg.lower() == p.value:
                return format_model_list(provider=p)
        # Otherwise: set model name
        save_cc_code_settings({"model": arg})
        return f"saved model={arg} to {CC_CODE_SETTINGS_PATH}\nRestart CC-Coder for the change to take effect."

    if user_input == "/user" or user_input.startswith("/user "):
        from cc_code.user_profile import handle_user_command
