...
CC-Coder components: ContextManager, Session, Agent Loop, Permissions,
...
from cc_code.context_manager import ContextManager, estimate_tokens
...
class TestMemoryPermissionIntegration:
    ...
    def test_memory_isolation_between_workspaces(self, tmp_workspace):
        ...
        assert (ws_a / ".cc-code-memory" / "memory.json").exists()
        assert (ws_b / ".cc-code-memory" / "memory.json").exists()

        a_path = ws_a / ".cc-code-memory" / "memory.json"
        b_path = ws_b / ".cc-code-memory" / "memory.json"
    ...
    def test_memory_file_path_security_no_path_traversal(self, tmp_workspace):
        ...
        assert local_path == tmp_workspace / ".cc-code-memory-local"
        assert project_path == tmp_workspace / ".cc-code-memory"
...
class TestMemoryRecoveryIntegration:
    ...
    def test_loading_from_corrupted_memory_file(self, tmp_workspace):
        memory_dir = tmp_workspace / ".cc-code-memory"
        memory_dir.mkdir()
