...
import cc_code.permissions as permissions_module
from cc_code.permissions import PermissionManager, _classify_dangerous_command, _is_within_directory


@pytest.fixture(autouse=True)
def isolated_permission_store(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    store_path = tmp_path / "home" / "permissions.json"
    monkeypatch.setattr(permissions_module, "CC_CODE_PERMISSIONS_PATH", store_path)
