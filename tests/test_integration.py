"""End-to-end integration tests for CC-Coder Python.
...
from cc_code.mock_model import MockModelAdapter
from cc_code.permissions import PermissionManager
from cc_code.tooling import ToolContext, ToolRegistry, ToolDefinition, ToolResult
from cc_code.tools import create_default_tool_registry
from cc_code.types import AgentStep, ChatMessage
from cc_code.context_manager import ContextManager
from cc_code.session import SessionData, save_session, load_session, list_sessions
from cc_code.config import load_effective_settings, CC_CODE_DIR
from cc_code.prompt import build_system_prompt
from cc_code.tui.types import TranscriptEntry, _create_transcript_entry, _recycle_transcript_entry
...
class TestAgentLoopIntegration:
    ...
    def test_edit_file_via_agent(
        ...
        system_messages.append({
            "role": "user",
            "content": "/edit hello.txt::Hello, world!::Hello, CC-Coder!",
        })
        ...
        assert "Hello, CC-Coder!" in content
...
class TestSessionIntegration:
    ...
    def test_session_save_load_roundtrip(self, tmp_workspace):
        ...
        # Cleanup
        session_path = CC_CODE_DIR / "sessions" / "test-integration-001.json"
    ...
    def test_session_listing(self, tmp_workspace):
        ...
        # Cleanup
        for i in range(3):
            path = CC_CODE_DIR / "sessions" / f"test-list-{i:03d}.json"
            if path.exists():
                path.unlink()
...
class TestLiveAPI:
    ...
    def test_simple_question(self, tools, tmp_workspace, auto_allow_permissions):
        ...
        from cc_code.anthropic_adapter import AnthropicModelAdapter
