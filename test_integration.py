...
# Ensure we can import cc_code
sys.path.insert(0, str(Path(__file__).parent))

print("=" * 70)
print("  CC-Coder - Integration Test")
print("=" * 70)
print()
...
def test_import_core():
    ...
    try:
        from cc_code import agent_loop, config, permissions, prompt, skills
        from cc_code import tooling, mcp, history, workspace
        return True
...
def test_import_new_features():
    ...
    try:
        from cc_code import state, cost_tracker, context_manager
        from cc_code import api_retry, task_tracker, memory
        from cc_code import poly_commands, async_context, sub_agents
        from cc_code import auto_mode, hooks, session, install
        return True
    except ImportError as e:
...
def test_store_state():
    ...
    from cc_code.state import Store, AppState, create_app_store
    ...
    # Update state
    from cc_code.state import set_busy, set_idle, update_context_usage
...
def test_task_tracker():
    """Test task tracking."""
    from cc_code.task_tracker import TaskManager
...
def test_memory_system():
    ...
    import tempfile
    from pathlib import Path
    from cc_code.memory import MemoryManager, MemoryScope
...
def test_poly_commands():
    ...
    from cc_code.poly_commands import (
        CommandRegistry, LocalCommand, CommandMetadata, create_builtin_commands
    )
    from cc_code.state import create_app_store
    from cc_code.cost_tracker import CostTracker
...
def test_sub_agents():
    ...
    from cc_code.sub_agents import (
...
def test_session_persistence():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("cc_code.session.SESSIONS_DIR", tmp_path / "sessions"), \
             patch("cc_code.session.CC_CODE_DIR", tmp_path):
...
def test_async_context():
    ...
    import asyncio
    from cc_code.async_context import AsyncContextCollector
